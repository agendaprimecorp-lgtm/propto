...
function analiseFalsa(over: Partial<PhotoAnalysis> = {}): PhotoAnalysis {
  return normalizeAnalysis({
    room_type: 'sala', quality_score: 0.88, issues: [],
    has_face: false, has_plate: false, faces: [], plates: [],
    caption: 'Sala com dois ambientes e varanda.', suggested_position: 1,
    ...over,
  });
}
...
describe('normalização da resposta do modelo', () => {
  test('caixa presente implica presença, mesmo com o booleano errado', () => {
    const a = normalizeAnalysis({
      ...
      plates: [], caption: 'x', room_type: 'sala', quality_score: 0.5,
    });
...
describe('worker no banco', { skip: temBanco ? false : 'SUPABASE_DB_URL não definido' }, () => {
  ...
  test('foto sem rosto é tratada e promovida a pronta', async () => {
    ...
    await storage.put('raw', `${orgId}/${propertyId}/limpa.jpg`, await foto(1200, 800, 1), 'image/jpeg');

    const out = await processMediaJob(
      deps(storage, new DetectorFalso(analiseFalsa())),
      { id: 'j1', org_id: orgId, type: 'process', payload: { media_id: mediaId }, attempts: 1 },
    );

    assert.equal(out.status, 'pronta');
    const { rows } = await pool.query(
      `select status, anonymized, exif_stripped, room_type, storage_path_public, phash, width
         from public.property_media where id = $1`, [mediaId],
    );
    const m = rows[0];
  ...
  test('foto com rosto é borrada antes de ir para o bucket público', async () => {
    ...
    await storage.put('raw', `${orgId}/${propertyId}/com-rosto.jpg`, await foto(1200, 800, 2), 'image/jpeg');
    ...
    assert.ok(storage.list().some((k) => k.startsWith('public/')), 'a derivada pública foi enviada');
  });

  test('análise indisponível NÃO deixa a foto passar sem blur', async () => {
    const storage = new MemoryStorage();
    const mediaId = await novaMidia('sem-analise.jpg');
    await storage.put('raw', `${orgId}/${propertyId}/sem-analise.jpg`, await foto(1200, 800, 3), 'image/jpeg');

    await assert.rejects(
      () => processMediaJob(
        deps(storage, new DetectorFalso(new Error('gateway fora do ar'))),
        { id: 'j3', org_id: orgId, type: 'process', payload: { media_id: mediaId }, attempts: 1 },
      ),
      /gateway fora do ar/,
    );
    ...
    assert.equal(storage.list().filter((k) => k.startsWith('public/')).length, 0,
  ...
  test('a mesma foto reenviada é marcada como duplicada', async () => {
    ...
    await processMediaJob(deps(storage, new DetectorFalso(analiseFalsa())),
      { id: 'j4', org_id: orgId, type: 'process', payload: { media_id: primeira }, attempts: 1 });

    const segunda = await novaMidia('dup-2.jpg');
    await storage.put('raw', `${orgId}/${propertyId}/dup-2.jpg`,
      await sharp(img).resize({ width: 640 }).jpeg({ quality: 70 }).toBuffer(), 'image/jpeg');
    const out = await processMediaJob(deps(storage, new DetectorFalso(analiseFalsa())),
      { id: 'j5', org_id: orgId, type: 'process', payload: { media_id: segunda }, attempts: 1 });

    assert.equal(out.status, 'descartada');
    assert.equal(out.duplicateOf, primeira);
    const { rows } = await pool.query(
      `select status, flagged_reason from public.property_media where id = $1`, [segunda],
    );
  ...
  test('duas fotos escuras diferentes NÃO viram duplicadas', async () => {
    ...
    await storage.put('raw', `${orgId}/${propertyId}/escura-1.jpg`, await fotoSemTextura(), 'image/jpeg');
    const r1 = await processMediaJob(deps(storage, new DetectorFalso(analiseFalsa())),
      { id: 'j7', org_id: orgId, type: 'process', payload: { media_id: a }, attempts: 1 });

    const b = await novaMidia('escura-2.jpg');
    await storage.put('raw', `${orgId}/${propertyId}/escura-2.jpg`,
      await fotoSemTextura(880, 590), 'image/jpeg');
    const r2 = await processMediaJob(deps(storage, new DetectorFalso(analiseFalsa())),
      { id: 'j8', org_id: orgId, type: 'process', payload: { media_id: b }, attempts: 1 });

    assert.equal(r1.status, 'pronta');
    assert.equal(r2.status, 'pronta',
      'sem textura o pHash não distingue nada — descartar seria perder foto do corretor');

    const { rows } = await pool.query(
      `select flagged_reason from public.property_media where id = $1`, [b],
    );
    assert.equal(rows[0].flagged_reason, 'escura',
  ...
  test('mídia de outra organização é recusada', async () => {
    ...
    await assert.rejects(
      () => processMediaJob(
        deps(storage, new DetectorFalso(analiseFalsa())),
        {
          id: 'j6', org_id: '00000000-0000-4000-8000-000000000000',
          type: 'process', payload: { media_id: mediaId }, attempts: 1,
        },
      ),
  ...
  test('a fila é consumida de ponta a ponta e o job falho vai para erro', async () => {
    ...
    await storage.put('raw', `${orgId}/${propertyId}/fila-ok.jpg`, await foto(700, 500, 5), 'image/jpeg');

    await pool.query(
      `insert into public.media_jobs (org_id, type, payload)
       values ($1, 'process', jsonb_build_object('media_id', $2::text))`, [orgId, ok],
    );
    const quebrado = await novaMidia('fila-erro.jpg');   // sem arquivo no storage
    await pool.query(
      `insert into public.media_jobs (org_id, type, payload)
       values ($1, 'process', jsonb_build_object('media_id', $2::text))`, [orgId, quebrado],
    );
    ...
    const r2 = await pool.query(
      `select status, error_message from public.property_media where id = $1`, [quebrado],
    );
    ...
    const jobs = await pool.query(
      `select status from public.media_jobs where org_id = $1 order by created_at`, [orgId],
    );
    const estados = jobs.rows.map((r: { status: string }) => r.status).sort();
    assert.deepEqual(estados, ['concluido', 'erro'],
